...
; 152: pide la prox tarea a ejecutar, y devuelve en ax el selector de segmento de esta

; 153: se asegura de no saltar a la tarea 0 (no se xq)

; 156: STR lee el registro TR y lo guarda en el registro pasado (bx)
; 157: se asegura que no vaya a saltar a la misma tarea que se esta ejecutando (compara el selector actual en bx con ax)

; 160: mueve el valor del selector de la nueva tarea a la posición de memoria reservada para el selector de tarea.
; 161: salta al contenido de la dirección sched_task_offset, o sea el selector de TSS en la GDT
; ???????????????????????????????????????????????????offsetayuda

; b) 48 bits xq lo decia la practica :-D (selector de 16 + offset de 32 ?). Ni idea... si??

; c) a la línea 163 (.fin)
